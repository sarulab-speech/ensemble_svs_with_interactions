...
class GaussianDiffusion(BaseModel):
    ...
    def forward(self, cond, lengths=None, y=None, spk_embs=None):
        """Forward step
        ...
        if self.encoder is not None:
            cond = self.encoder(cond, lengths, spk_embs=spk_embs)
    ...
    def inference(self, cond, lengths=None, spk_embs=None):
        B = cond.shape[0]
        device = cond.device
...
class MultiSpeakerGaussianDiffusion(GaussianDiffusion):
    def __init__(
        self,
        in_dim,
        out_dim,
    ...
    def forward(self, cond, spks, lengths=None, y=None):
        """Forward step
        ...
        spk_embs = spk_embs.expand(spk_embs.shape[0], cond.shape[1], spk_embs.shape[-1])
    ...
    def inference(self, cond, spks, lengths=None):
        B = cond.shape[0]
        device = cond.device
