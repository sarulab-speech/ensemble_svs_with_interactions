...
from nnsvs.data import (
    ...
    MelF0AcousticSource,
    MultiTrackMusicalLinguisticSource,
...
def _prepare_timelag_feature(
    ...
    tpath = join(in_timelag_root, name + "-times.npy")
    np.save(xpath, x, allow_pickle=False)
    np.save(ypath, y, allow_pickle=False)
    np.save(tpath, abs_time, allow_pickle=False)
...
def _prepare_duration_feature(
    ...
    tpath = join(in_duration_root, name + "-times.npy")
    np.save(xpath, x, allow_pickle=False)
    np.save(ypath, y, allow_pickle=False)
    np.save(tpath, abs_time, allow_pickle=False)
...
def _prepare_acoustic_feature(
    ...
    tpath = join(in_acoustic_root, name + "-times.npy")
...
def my_app(config: DictConfig) -> None:
    ...
    in_timelag_source = MultiTrackMusicalLinguisticSource(
        utt_list,
    ...
    in_duration_source = MultiTrackMusicalLinguisticSource(
        utt_list,
    ...
    if config.acoustic.question_path is not None:
        ...
        question_path = question_path_general
    in_acoustic_source = MultiTrackMusicalLinguisticSource(
        utt_list,
    ...
    if config.timelag.enabled:
        logger.info(
            "Timelag linguistic feature dim: %s", str(in_timelag[0][0].shape[1])
        )
    ...
    if config.duration.enabled:
        logger.info(
            "Duration linguistic feature dim: %s", str(in_duration[0][0].shape[1])
        )
    ...
    if config.acoustic.enabled:
        logger.info(
            "Acoustic linguistic feature dim: %s", str(in_acoustic[0][0].shape[1])
        )
