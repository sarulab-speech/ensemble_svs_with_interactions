...
from nnsvs.train_util import (
    ...
    collate_fn_syncmultitrack_acoustic,
...
def train_step(
    ...
    out_scaler,
    feats_criterion="mse",
    pitch_reg_dyn_ws_list=(1.0, 1.0),
    pitch_reg_weight=1.0,
    logf0_diff_weight=1.0,
    mgc_diff_weight=1.0,
    sub_require_grad=True,
    ...
    with autocast(enabled=grad_scaler is not None):
        outs = model(
            in_feats0,
            in_feats1,
            spks_list=tuple(spks_list),
            lengths=lengths,
            ys=out_feats_list,
        )
        if model.has_residual_lf0_prediction():
            (pred_out_feats_main, lf0_residual_main), (
                pred_out_feats_sub,
                lf0_residual_sub,
            ) = outs
        else:
    ...
    if sub_require_grad == False:
        pred_out_feats_sub = pred_out_feats_sub.detach()
    ...
    if prediction_type == PredictionType.MULTISTREAM_HYBRID:
        loss_feats = 0
        streams_main = split_streams(out_feats_list[0], model_config.stream_sizes)
        streams_sub = split_streams(out_feats_list[1], model_config.stream_sizes)
        assert len(streams_main) == len(pred_out_feats_main)
        if stream_wise_loss:
            weights = get_stream_weight(stream_weights, model_config.stream_sizes).to(
                in_feats.device
            )
        else:
            weights = [None] * len(streams_main)
            N = 0

        for pred_stream, stream, sw in zip(pred_out_feats_main, streams_main, weights):
            # DDPM
        ...
        if logf0_diff_weight > 0.0:
            vuv_flag = ((streams_main[2] > 0)) & ((streams_sub[2] > 0))
            pred_lf0_diff = pred_out_feats_main[1] - pred_out_feats_sub[1]
            lf0_diff = streams_main[1] - streams_sub[1]
            loss_lf0_interaction = criterion(
                pred_lf0_diff.masked_select(mask & vuv_flag),
                lf0_diff.masked_select(mask & vuv_flag),
            ).mean()

        assert torch.isfinite(loss_feats)
    elif prediction_type == PredictionType.PROBABILISTIC:
        assert False
        # not supported
        pi, sigma, mu = pred_out_feats_main
        ...
        with autocast(enabled=grad_scaler is not None):
            ...
            if not isinstance(pred_out_feats_main, list):
                pred_out_feats_main = [pred_out_feats_main]
            if not isinstance(pred_out_feats_sub, list):
                pred_out_feats_sub = [pred_out_feats_sub]

            loss_feats = 0
            for pred_out_feats_main_, pred_out_feats_sub_ in zip(
                pred_out_feats_main, pred_out_feats_sub
            ):
                if stream_wise_loss:
                    weights = get_stream_weight(
                        stream_weights, model_config.stream_sizes
                    ).to(in_feats.device)
                    streams_main = split_streams(
                        out_feats_list[0], model_config.stream_sizes
                    )
                    streams_sub = split_streams(
                        out_feats_list[1], model_config.stream_sizes
                    )
                    pred_streams_main = split_streams(
                        pred_out_feats_main_, model_config.stream_sizes
                    )
                    pred_streams_sub = split_streams(
                        pred_out_feats_sub_, model_config.stream_sizes
                    )
                else:
                    streams_main = [out_feats_list[0]]
                    streams_sub = [out_feats_list[1]]
                    pred_streams_main = [pred_out_feats_main_]
                    pred_streams_sub = [pred_out_feats_sub_]
                    weights = [1.0]
                for pred_stream, stream, sw in zip(
                    pred_streams_main, streams_main, weights
                ):
                    with autocast(enabled=grad_scaler is not None):
                        loss_feats += (
                            sw
                ...
                if logf0_diff_weight > 0.0:
                    # vuv_flag = ((streams_main[0][...,2:3] > 0)) & ((streams_sub[0][...,2:3] > 0))
                    vuv_flag = ((streams_main[0][..., 61:62] > 0)) & (
                        (streams_sub[0][..., 61:62] > 0)
                    )
                    pred_lf0_diff = (
                        pred_out_feats_main_[..., 60:61]
                        - pred_out_feats_sub_[..., 60:61]
                    )
                    lf0_diff = streams_main[0][..., 60:61] - streams_sub[0][..., 60:61]
                    loss_lf0_interaction = criterion(
                        pred_lf0_diff.masked_select(mask & vuv_flag),
                        lf0_diff.masked_select(mask & vuv_flag),
                    ).mean()

                if mgc_diff_weight > 0.0:
                    pred_mgc0th_diff = (
                        pred_out_feats_main_[..., 0:1] - pred_out_feats_sub_[..., 0:1]
                    )
                    mgc0th_diff = streams_main[0][..., 0:1] - streams_sub[0][..., 0:1]
                    loss_mgc0th_interaction = criterion(
                        pred_mgc0th_diff.masked_select(mask),
                        mgc0th_diff.masked_select(mask),
                    ).mean()
    ...
    if pitch_reg_weight > 0.0 and lf0_residual_main is not None:
        with autocast(enabled=grad_scaler is not None):
            if isinstance(lf0_residual_main, list):
                loss_pitch = 0
                for lf0_residual_ in lf0_residual_main:
                    loss_pitch += (
                ...
                loss_pitch = (
                    (pitch_reg_dyn_ws * lf0_residual_main.abs())
                    .masked_select(mask)
                    .mean()
                )
    else:
    ...
    if not "loss_lf0_interaction" in locals():
        loss_lf0_interaction = torch.tensor(0.0).to(in_feats.device)
    if not "loss_mgc0th_interaction" in locals():
        loss_mgc0th_interaction = torch.tensor(0.0).to(in_feats.device)
    loss = (
        loss_feats
        + pitch_reg_weight * loss_pitch
        + logf0_diff_weight * loss_lf0_interaction
        + mgc_diff_weight * loss_mgc0th_interaction
    )

    if not train:
        with torch.no_grad():
            if prediction_type == PredictionType.MULTISTREAM_HYBRID:
                if len(pred_out_feats_main) == 4:
                    pred_mgc, pred_lf0, pred_vuv, pred_bap = pred_out_feats_main
                    ...
                    if isinstance(pred_bap, tuple) and len(pred_bap) == 3:
                        ...
                        # Diffusion case: noise
                        pred_bap = pred_bap[1]
                    pred_out_feats_main_ = torch.cat(
                        [pred_mgc, pred_lf0, pred_vuv, pred_bap], dim=-1
                    )
                elif len(pred_out_feats_main) == 3:
                    pred_mel, pred_lf0, pred_vuv = pred_out_feats_main
                    if isinstance(pred_lf0, tuple) and len(pred_lf0) == 3:
                        pred_lf0 = mdn_get_most_probable_sigma_and_mu(*pred_lf0)[1]
                    elif isinstance(pred_lf0, tuple) and len(pred_lf0) == 2:
                        # Diffusion case: noise
                    ...
                    if isinstance(pred_mel, tuple) and len(pred_mel) == 3:
                        ...
                        pred_mel = pred_mel[1]
                    pred_out_feats_main_ = torch.cat(
                        [pred_mel, pred_lf0, pred_vuv], dim=-1
                    )
                else:
                    raise RuntimeError("not supported")
            elif prediction_type == PredictionType.PROBABILISTIC:
                pred_out_feats_main_ = mdn_get_most_probable_sigma_and_mu(
                    pi, sigma, mu
                )[1]
            else:
                if isinstance(pred_out_feats_main, list):
                    pred_out_feats_main_ = pred_out_feats_main[-1]
                else:
                    pred_out_feats_main_ = pred_out_feats_main
            distortions = compute_distortions(
                pred_out_feats_main_,
                out_feats_list[0],
                lengths,
                out_scaler,
                model_config,
    ...
    log_metrics.update(
        {
            ...
            "Loss_LogF0_Interaction": loss_lf0_interaction.item(),
            "Loss_MGC-0th_Interaction": loss_mgc0th_interaction.item(),
        }
    )
...
def train_loop(
    ...
    logf0_diff_weight = config.train.logf0_diff_weight
    mgc_diff_weight = config.train.mgc_diff_weight
    sub_require_grad = config.train.sub_require_grad
    ...
    for epoch in tqdm(range(1, config.train.nepochs + 1)):
        for phase in data_loaders.keys():
            ...
            for (
                ...
                spks1,
                lengths1,
            ) in tqdm(data_loaders[phase], desc=f"{phase} iter", leave=False):
                ...
                with torch.no_grad():
                    lf0_score_denorm0 = (
                        in_feats0[:, :, in_lf0_idx] - in_scaler.min_[in_lf0_idx]
                    ) / in_scaler.scale_[in_lf0_idx]
                    lf0_score_denorm1 = (
                        in_feats1[:, :, in_lf0_idx] - in_scaler.min_[in_lf0_idx]
                    ) / in_scaler.scale_[in_lf0_idx]
                    # Fill zeros for rest and padded frames
                    lf0_score_denorm0 *= (in_feats0[:, :, in_rest_idx] <= 0).float()
                    lf0_score_denorm0[make_pad_mask(lengths0)] = 0
                    lf0_score_denorm1 *= (in_feats1[:, :, in_rest_idx] <= 0).float()
                    lf0_score_denorm1[make_pad_mask(lengths1)] = 0

                    # Compute time-variant pitch regularization weight vector
                    # NOTE: the current impl. is very slow
                    if pitch_reg_weight > 0.0:
                        pitch_reg_dyn_ws0 = compute_batch_pitch_regularization_weight(
                            lf0_score_denorm0,
                            decay_size=config.train.pitch_reg_decay_size,
                        )
                        pitch_reg_dyn_ws1 = compute_batch_pitch_regularization_weight(
                            lf0_score_denorm1,
                            decay_size=config.train.pitch_reg_decay_size,
                        )
                    else:
                        pitch_reg_dyn_ws0 = 1.0
                        pitch_reg_dyn_ws1 = 1.0
                ...
                if (not train) and (not evaluated):
                    eval_model(
                        ...
                        epoch,
                        model,
                        in_feats=(in_feats0, in_feats1),
                        out_feats=[out_feats0, out_feats1],
                        spks=(spks0, spks1),
                        ...
                        writer=writer,
                        sr=sr,
                        lf0_score_denorm=(lf0_score_denorm0, lf0_score_denorm1),
                ...
                if logf0_diff_weight == "exponential":
                    logf0_diff_weight = 2.0 ** ((epoch - config.train.nepochs) / 10)
                if mgc_diff_weight == "exponential":
                    mgc_diff_weight = 2.0 ** ((epoch - config.train.nepochs) / 10)
                loss, log_metrics = train_step(
                    ...
                    pitch_reg_dyn_ws_list=(pitch_reg_dyn_ws0, pitch_reg_dyn_ws1),
                    pitch_reg_weight=pitch_reg_weight,
                    logf0_diff_weight=logf0_diff_weight,
                    mgc_diff_weight=mgc_diff_weight,
                    sub_require_grad=sub_require_grad,
                )
...
def my_app(config: DictConfig) -> None:
    if "max_time_frames" in config.data and config.data.max_time_frames > 0:
        ...
        if "reduction_factor" in config.model.netG:
            ...
            collate_fn = collate_fn_syncmultitrack_acoustic
    ...
    in_scaler = PyTorchMinMaxScaler(
        torch.from_numpy(in_scaler.min_), torch.from_numpy(in_scaler.scale_)
    ).to(device)
