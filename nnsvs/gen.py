...
def predict_timelag(
    ...
    if timelag_model.prediction_type() == PredictionType.PROBABILISTIC:
        # (B, T, D_out)
        max_mu, max_sigma = timelag_model.inference(x, spks=spk, lengths=x.shape[1])
        ...
        # (T, D_out)
        pred_timelag = (
            timelag_model.inference(x, spks=spk, lengths=x.shape[1])
            .squeeze(0)
            .cpu()
            .data.numpy()
        )
    ...
    return pred_timelag * hts_frame_shift, pred_timelag


@torch.no_grad()
def predict_timelag_multitrack(
    device,
    labels_list,
    spks_list,
    timelag_model,
    ...
    for labels in labels_list:
        ...
        start_timings_list.append(note_labels.start_times)
        ...
        x = torch.from_numpy(timelag_linguistic_features).unsqueeze(0).to(device)
        x_list.append(x)
        note_labels_list.append(note_labels)
        original_lengths.append(x.shape[1])
    ...
    a = start_timings_list[0]
    b = start_timings_list[1]
    inf = a[-1] + b[-1]
    ...
    aid = 0
    bid = 0
    new_x0 = torch.empty((0, len(x0[0])), dtype=torch.float32, device=x0.device)
    new_x1 = torch.empty((0, len(x1[0])), dtype=torch.float32, device=x1.device)

    while aid < len(a) - 1 or bid < len(b) - 1:
        if a[aid] < b[bid]:
            new_x0 = torch.vstack([new_x0, x0[aid]])
            new_x1 = torch.vstack([new_x1, torch.zeros_like(x0[aid])])
            mask0 = torch.cat((mask0, torch.BoolTensor([True])))
            mask1 = torch.cat((mask1, torch.BoolTensor([False])))
            aid += 1
        elif a[aid] > b[bid]:
            new_x0 = torch.vstack([new_x0, torch.zeros_like(x1[bid])])
            new_x1 = torch.vstack([new_x1, x1[bid]])
    ...
    max_len = max(len(x0), len(x1))
    x0 = torch.from_numpy(pad_2d(x0.cpu(), max_len)).unsqueeze(0).to(device)
    x1 = torch.from_numpy(pad_2d(x1.cpu(), max_len)).unsqueeze(0).to(device)

    x = torch.concat((x0, x1), dim=2)
    # Run model
    if timelag_model.prediction_type() == PredictionType.PROBABILISTIC:
        # (B, T, D_out)
        # max_mu, max_sigma = timelag_model.inference(x, spks=tuple(spks_list), lengths=original_lengths[0])
        max_mu, max_sigma = timelag_model.inference(x, spks=tuple(spks_list))
        if np.any(timelag_config.has_dynamic_features):
            # Apply denormalization
            # (B, T, D_out) -> (T, D_out)
            max_sigma_sq = (
    ...
    # Rounding
    pred_timelag = pred_timelag[mask0]
    eval_pred_timelag = pred_timelag
    pred_timelag = np.round(pred_timelag)

    # Clip to the allowed range
    for idx in range(len(pred_timelag)):
        if _is_silence(note_labels_list[0].contexts[idx]):
            pred_timelag[idx] = np.clip(
                pred_timelag[idx], allowed_range_rest[0], allowed_range_rest[1]
            )
        else:
    ...
    return pred_timelag * hts_frame_shift, eval_pred_timelag, mask0
...
def predict_duration(
    ...
    if duration_model.prediction_type() == PredictionType.PROBABILISTIC:
        # (B, T, D_out)
        max_mu, max_sigma = duration_model.inference(x, spks=spk, lengths=x.shape[1])
        ...
        # (T, D_out)
        pred_durations = (
            duration_model.inference(x, spks=spk, lengths=x.shape[1])
            .squeeze(0)
            .cpu()
            .data.numpy()
        )
...
@torch.no_grad()
def predict_duration_multitrack(
    device,
    labels_list,
    spks_list,
    duration_model,
    ...
    for labels in labels_list:
        ...
        start_timings_list.append(labels.start_times)
    ...
    if duration_model.prediction_type() == PredictionType.PROBABILISTIC:
        # (B, T, D_out)
        max_mu, max_sigma = duration_model.inference(x, spks=tuple(spks_list))
        if np.any(duration_config.has_dynamic_features):
            raise RuntimeError(
                "Dynamic features are not supported for duration modeling"
            )
        ...
        max_mu = max_mu[mask0]
        max_sigma_sq = max_sigma_sq[mask0]
        return max_mu, max_sigma_sq
    else:
        assert False
        # (T, D_out)
...
def postprocess_duration(labels, pred_durations, lag, frame_period=5):
    ...
    for i in range(1, len(note_indices)):
        ...
        d_norms += np.cumsum(d_norm.reshape(-1)).tolist()

        if len(output_labels) > 0:
            output_labels.end_times[-1] = p.start_times[0]
        for n in p:
            output_labels.append(n)
    return output_labels, np.array(d_norms)
...
def predict_timing(
    ...
    # Time-lag
    lag, lag_for_eval = predict_timelag(
        device=device,
        labels=labels,
        spk=spk,
    ...
    duration_modified_labels, d_norms = postprocess_duration(labels, durations, lag)
    return duration_modified_labels, lag_for_eval, d_norms


def predict_timing_multitrack(
    device,
    labels_list,
    spks_list,
    binary_dict,
    ...
    # Time-lag
    lag, lag_for_eval, mask = predict_timelag_multitrack(
        device=device,
    ...
    # Duration predictions
    durations = predict_duration_multitrack(
        device=device,
        labels_list=labels_list,
        spks_list=spks_list,
        duration_model=duration_model,
    ...
    # Normalize phoneme durations
    duration_modified_labels, d_norms = postprocess_duration(
        labels_list[0], durations, lag
    )
    # print(len(labels_list[0]), len(labels_list[1]), lag.shape, durations[0].shape, len(duration_modified_labels))
    return duration_modified_labels, lag_for_eval, d_norms, mask
...
def predict_acoustic(
    ...
    if acoustic_model.prediction_type() in [
        ...
        # (B, T, D_out)
        if spk is not None:
            max_mu, max_sigma = acoustic_model.inference(
                x, spks=spk, lengths=[x.shape[1]]
            )
        else:
            max_mu, max_sigma = acoustic_model.inference(x, lengths=[x.shape[1]])
        ...
        if spk is not None:
            pred_acoustic = (
                acoustic_model.inference(x, lengths=[x.shape[1]], spks=spk)
                .squeeze(0)
                .cpu()
                .data.numpy()
            )
            ...
            pred_acoustic = (
                acoustic_model.inference(x, lengths=[x.shape[1]])
                .squeeze(0)
                .cpu()
                .data.numpy()
            )
...
@torch.no_grad()
def predict_acoustic_multitrack(
    device,
    labels_list,
    spks_list,
    acoustic_model,
    ...
    if acoustic_model.prediction_type() in [
        ...
        # (B, T, D_out)
        max_mu, max_sigma = acoustic_model.inference(
            x0, x1, spks=spks_list, lengths=[x0.shape[1]]
        )
        if np.any(acoustic_config.has_dynamic_features):
            # Apply denormalization
            # (B, T, D_out) -> (T, D_out)
        ...
        # (T, D_out)
        # padding to make T(main) to T(sub)
        max_len = max(x0.shape[1], x1.shape[1])
        x0 = torch.from_numpy(pad_2d(x0.squeeze(), max_len, 0)).unsqueeze(0).to(device)
        x1 = torch.from_numpy(pad_2d(x1.squeeze(), max_len, 0)).unsqueeze(0).to(device)

        pred_acoustic = (
            acoustic_model.inference(
                x0, x1, spks=tuple(spks_list), lengths=[x0.shape[1]]
            )
            .squeeze(0)
            .cpu()
            .data.numpy()
...
def postprocess_waveform(
    ...
    wav /= np.max(np.abs(wav))  # added to avoid codec error
