...
for spk in speakers:
    ...
    for align_path, score_path in zip(align_lab_files, score_lab_files):
        ...
        assert len(align_mono_lab) == len(score_full_lab)
        ...
        if has_too_short_ph is False:
            ...
            with open(join(full_score_dir, name + ".lab"), "w") as of:
                of.write(str(new_score_full_lab))
            with open(join(full_align_dir, name + ".lab"), "w") as of:
                of.write(str(align_full_lab))

    # divide .lab into small segments
    base_files = sorted(glob(join(out_dir, "label_phone_align", "*.lab")))

    lengths = {}

    for name in ["label_phone_align", "label_phone_score"]:
        files = sorted(glob(join(out_dir, name, "*.lab")))
        for idx, base in tqdm(enumerate(base_files)):
            ...
            # keep lengths of each segment for logging
            if name == "label_phone_align":
                d = []
            ...
            dst_dir = join(out_dir, f"{name}_seg")
            os.makedirs(dst_dir, exist_ok=True)
            for idx, seg in enumerate(segments):
                with open(join(dst_dir, f"{utt_id}_seg{idx}.lab"), "w") as of:
                    of.write(str(seg))

            # base_dst_dir = join(out_dir, "mono_label_round_seg")
